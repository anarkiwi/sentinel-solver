...
Three parts: a bit-exact forward model (`sentinel/`), a driver that executes against the
...
Rules and their ROM addresses: [gameplay.md](gameplay.md). Players:
[players.md](players.md). Everything still wrong: [open_items.md](open_items.md).
...
- **A u-turn unfreezes the world mid-aim.** `$12D5 CMP #$22 / BCS $12DE` lets codes >=
...
`plot_world` frame and 400 enemy rounds (~1.1M instructions, 90 distinct opcodes) hits none
of the 105 illegal opcodes, so a 6502 interpreter is a sound instrument. The frame clock is
not golden-pinned; the instrument gates it.
...
- **PRNG-driven landing coordinates.** `actions.hyperspace` (`do_hyperspace $2156`) is
  ...
  deliberately unread. The draw *rate* is unmodellable (the ROM draws ~19+ times a frame,
  often more than 32, while the cursor moves 3). This limits exactly two things, both
  through `put_object_in_random_tile_below_z $1224`: the discharge tree's tile and the
  hyperspace tile. Meanie creation is **not** one — `$197D` is a deterministic slot scan,
  as are the hunt and the hyperspace trigger.
...
- **U-turn** = one action tap (`UTURN_FRAMES = 74`, measured live ls42), 0 scroll frames,
...
- `HOP_FRAMES = 700` is the window a full hop (2 creates + transfer + aims) needs (live
  ls42 hops measured 745 and 879 f whole-step); `SAFE_FRAMES = 250` is the window below
  which a tile is urgent.

Live ls42 whole-step charged-vs-measured: **rms 24.1 f, mean −12.0, max |e| 46, n=11**
(`live_ls42_hops.json`, `test_hop_budget.py`).
...
**Wrap safety.** The ROM compares z as bytes, so a ray more than `0xFF80` above a surface
aliases onto "equal" (measured max `zrel` 472 on the `$F5` plane, 95976 on the band;
168,880 band rays exceed the alias distance at a non-origin cell). Those visits are kept
unconditionally (wildcards, and `crossing_mask`'s `wrap_z` branch), ~490 extra rays per
arc-narrowed query.

**Measured.** Lattices are `los._landable_sweep`'s: the `$F5` **plane** (32 h × 64 cx × 128
cy = 262,144 rays) and the full **band** (× 27 pitches = 3,538,944 rays), `max_steps` 6000.
Track census (`python -m sentinel.landtable`, 24 threads, 6.7 s for both): 43.7/43.3
distinct cells per ray, 11.5 M/124.6 M (ray, cell) visits, 0/1.81 M wildcards, candidates
per cell at `T <= 0` mean 3035 (p50 2138)/mean 20103 (p50 12982). Over 3775 targeted
queries from real solves (ls0/42/335), `T = surface16 - eye_z*256 - obj_z_frac` spans −1120
to +1568, eye heights 5..11, `|dx|`/`|dy|` up to 30:

| | rays marched, band | plane | query cost, band | plane |
|---|---|---|---|---|
| arc only | mean 77393, p50 50367 | mean 5648, p50 3584 | 15.46 ms | 1.24 ms |
| arc + crossing (`landable_view`) | mean 18994, p50 2010 | mean 2267, p50 578 | **3.20 ms** | **0.92 ms** |

1638 of the 2817 ls42 queries come back as a *proven* "no view". The expensive residual is
an **adjacent** cell: its arc is huge and a ray dwells long enough inside it to cross almost
any surface height, so ~70% of the arc survives the filter (worst 623 k of 886 k rays).
...
tile(s) it can stop in, partitioning the lattice by landing tile. Rays crossing no surface
band before leaving the board (77-88% of the lattice) are never marched. Over 7 stances
that is 5.5-9× fewer rays and ~3× less wall time (ls0, 49 tiles: 83.8 ms / 884,736 rays →
25.6 ms / 152,025), exact tile for tile including the ring; per-tile queries on the same
lattice go 21,660 → 4,372 rays (p50 14,698 → 776), 2.67 → 0.55 ms. Soundness needs one
extra step: the walk may only stop where the ray **provably** stops (`zmin <= surface_lo`);
an undecidable cell (object stack with `lo < hi`, or alias risk) makes the ray
multi-candidate and always marched (`test_stop_cell_partition_holds_every_landing`).
...
Addresses are ROM `$hex`; PAL frame = 19656 cycles. Validated against
...
`plot_tile $2A24` → `prepare_polygon $2D6C` / `process_lines`+`process_line $2DF2`/`$3002`
/ `span_fill $22AA` / `plot_middle_of_row $23D0`, object tiles adding
`plot_stack_of_objects $21AE`. Before the replot, `populate_tile_visibility_bit_table
$245B` (from `$35BA`) raytraces occlusion into the `$3E80`/`$24DA` bitmap.
...
`screen_x` = high byte of `((h_angle16 + $0011:$0029) << 3)`; reproduces `$A7A0`/`$0B40` on
all 3574 swept vertices and the ported cycle sum equals the ROM `conv` bucket exactly,
258628 cyc, including the double-coordinate restart when any `h_angle16+$0011 >= $20`).
Edge build is per-polygon independent and the DDA edge walk reproduces the `$AD00`/`$AE00`
writes byte-for-byte on 534/534 narrow polygon-sections. Per-block costs come from the loop
...
`prepare_polygon` ~600 cyc/call (`C_PREP_CALL`). The fill is **prepare-dominated**: 5 of
the 15 views fill zero pixels yet spend 3k-450k cyc of terrain "fill" — pure `process_line`
edge tracing for polygons clipping out of the band. `prepare_polygon` runs per polygon × 2
wide-buffer sections (`$0010=0 < 2` at `$2AAB`), and a flat tile is one quad while a sloped
tile is two triangles (`plot_two_triangles $2A8A`), so a plotted tile costs 2-4 calls even
when nothing fills. Why the per-tile residual cannot close: [open_items.md](open_items.md).
...
engine facts `$9CA0`/`$9CA1` (verts) and `$9CAB`/`$9CAC` (polys): type 0=(29,27) 1=(22,25)
2=(17,15) 3=(8,10) 4=(18,25) 5=(30,35) 6=(12,11) 7=(8,4). An in-view object costs a
**63k-95k base** plus distance-dependent fill up to ~213k when close; `_inview_object_base`
sums that base over plotted object-tiles' `$0100` stacks and, with object `span_fill`
...
`RENDER_SECTIONS`) but are ROM-derived: a perturbation smaller than the model's own error
can flip a knife-edge board, so tuning them to win one is evidence of nothing.
...
(`test_transfer_tune_is_96_frames`). `SETTLE_FIXED_FRAMES ≈ 176` is a fitted stand-in for
four foreground routines absent from `render_cost`, py65 cycle-counted (`/19656`):

| routine | ROM | ls42 | ls335 |
| --- | --- | --- | --- |
| occlusion raytrace | `$245B` | 110f | 63f |
| grid angle/hypotenuse pass | `$3700` | 82f | 82f |
| `fill_screen_with_background` | `$1090` | ~1f | ~1f |
| `plot_status_bar` | `$98B2` | 7f | 7f |
| **sum** | | **199f** | **152f** |

Occlusion cost is scene-dependent, so 176 is the mean of these two scenes; raster-IRQ steal
(~10-25%) is folded into it and the tune base.
`test_viewpoint_replot_lands_in_live_settle_band` asserts each prediction lands in
`[0.75*lo, 1.25*hi]` of the recorded live band with median abs error < 15%; that band is
ls42 (338, 305, 435, 460), ls335 (259, 333, 371). It was measured through a 6 s wall-clock
`run_until_pc` that caps a reading at ~300 frames, so any value at or under ~300 is
indistinguishable from that ceiling. A u-turn scrolls 0 frames and is not a viewpoint
replot; `_exact_render_cost` returns `None` for any `observer != state.player`.
...
(288 notches over ls0/42/335). Measured notch cost spans **3.8 to 99.8 frames** (median
22.2), a swing no flat base covers: `test_pan_notch_cost_matches_the_measured_plot` pins
rms < 9 f, median |error| < 6 f, |bias| < 3 f, and
`test_derived_notch_beats_the_flat_base_it_replaced` requires under half the rms of the
best flat constant. The residual is the fill proxy, not
the notch model (measured mean 1704 cyc/examine vs 1737 charged) — do not add a
compensating constant to `pancost`. ~62% of a `render_cost` call was the view-independent
`$245B` raytrace, now memoized per (scene, observer) as `projector.occlusion_visible`, and
`notch_frames` per (scene, observer, direction, plot angle), both keyed off
...
`/renders/...`. The monitor is frame-quantized while the CPU runs — 0.04 ms halted, ~1.4 ms
under warp, ~23.5 ms at real-time pace, independent of read size — so read while halted and
treat a multi-second timeout as a wait on a PC that can never recur, not back-pressure.
With warp off, any dead dwell in which the CPU runs is live time in which the Sentinel can
spawn a ring meanie. Full-image reads are done in two 32 KB halves because `mem_get`'s
response length is a u16.
...
[open](open_items.md). The action-cost bill is **0.993×** the measured span between genuine
player actions — a correct bill lands under the span, since the human's think time sits on
the measured side. Applying the action last in its span (a bracket fires when the action
lands), 83 of 91 exact-span actions reproduce the human's next energy; the misses are off by
exactly one in both directions, i.e. drain-timing scatter inside the span. A drain does not
decrement a counter — `$1A08` **downgrades** its target (robot → boulder → tree → gone) —
so an absorb whose object was drained mid-span yields one less.

**`$0C30` is not a score.** The recorded `update_cooldown` sits on its stick value 1 in 77%
of ls335 `watch_play` samples (async, free-running machine) but 8% of ls42 `replay_human`
...
**Fixture hygiene.** The dither loop (`$1FA4`, ~50 f) and the transfer tune wait (`$35D5`,
96 f) are hard floors on how close two real actions can be; 8 exact ls335 spans fall below
the floor for the action preceding them, so those bracket pairs are one action recorded
twice. ls335 also carries 33 events of the two known recorder classes (enemy discharge
trees, drain ticks minted as self-transfers); `human_replay` skips them.
...
handovers concurrently, so 156 events settle in about three rounds (`--linear` keeps the
exhaustive backward scan, `--indices` runs a list). Attempts run one **spawned** process per
index (a fork aborts: the numba LOS march leaves an OpenMP runtime in the parent) and a
...
`parallel=True` `march_batch` takes a thread per core (10 workers put load at 117 on 24
threads and inflated attempts ~8×). Outcomes are `won`/`lost`/`capped`; `capped` means
undecided, and the top capped index is re-run alone at `escalate` × the cap before being
called.
...
makes replay mutate the checkpoint. A whole run is 36 ticks, 88 KB compressed.

| tier | question | measured |
|---|---|---|
| 0 filter tally | which gate kills each `(tile, k)` here? | 0-5 ms |
| 1 probe | does the candidate generator (`_climb_candidates`/`_mount`) yield anything here? | 140-210 ms |
...
Measured on a reduced ls335 board, enemies `{(4,18), (12,10)}` plus the Sentinel. Promote a
change only when the tier below passes: a generator change is judged in 210 ms against the
stance that defeats it, not 94 s (494 s under load). The fidelity gate — restoring at tick
*t* and replaying to the end reproduces the run's own trace tail entry for entry — must
