...
game's own memory. It is solver-independent — it imports only `sentinel/` (for
the bit-exact LOS/aim oracle and the memory map), never `solver/`; the runners
in `scripts/` wire a plan into it.
...
| `driver/sentinel_state.py` | read live 64 KB memory into a structured `GameState` (`ViceSource`/`Py65Source`), `verify_entry` against the standalone generator, `mem_image` for wrapping in a sim `State`/`PlanGame`. |
...
- **Full-image reads.** The simulator is defined over the full 64 KB image (enemy
  ...
  comes back empty). A planner resync only needs the first 4 KB, where all mutable
  play state lives.

## Relation to the offline plan

The driver executes an offline, aim-exact plan produced by `solver/`: each step
is `{verb, otype, target tile, view}`, consumed by `perform_step`. A step whose
`view` is `None` is a **deferred aim** — an on-boulder synthoid that must re-aim
after the boulder just landed, or an absorb whose coarse sweep did not resolve a
view — re-proposed here against **current** live memory via the shared aim
proposer (`sentinel.aim.propose` at the player's true eye), the same proposer the
planner and sim use, so the three never diverge on how a tile is aimed.

A **missed aim is treated as a crash** (`aim_miss`). The offline plan is
aim-exact, so a miss means the model diverged from the real game and must be
investigated, never smoothed over with margin — the live runner resyncs from true
memory and re-plans on a world-divergence, but a genuine wrong-tile landing is the
hard failure the live contract raises on. See [planner.md](planner.md) for the
offline-plan / live-reschedule contract.
