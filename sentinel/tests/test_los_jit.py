...
def test_jit_matches_python_bare(seed, max_steps):
    g = Game.new(seed)
    bad = _sweep_mismatches(g.state, g.state.player, None, max_steps)
    assert not bad, f"seed {seed} ms {max_steps}: {bad[:3]}"


@pytest.mark.parametrize("seed", SEEDS)
def test_jit_matches_python_with_objects(seed):
    """Objects placed around the player force the OBJECT-bail path (the ray lands
    on object tiles resolved in Python)."""
    g = Game.new(seed)
    st = g.state
    px, py = st.player_xy()
    for dx, dy, typ in [
        ...
        if 0 <= x < mm.N and 0 <= y < mm.N:
            try:
                ...
                pass
    bad = _sweep_mismatches(st, st.player, None, 20000)
