...
# 18 joined when the u-turn price rose from 74 to 77 (both since retired: settlecost now prices a u-turn per scene) and left again when the examine moved to the play machine's own $37F2: its budget is aim + HOP_FRAMES, and either way it is the drain window's edge that decides, not the step.
