"""The ONE player-action aiming/LOS layer, used by the live driver and the model.

Every player create/absorb resolves its aim through the two primitives here, so
callers can never diverge on *how* a tile is aimed or *whether* it is visible.  Both
are the ROM's own action path (``handle_player_actions`` $1B40):
...
otherwise rounded eye fabricates line of sight the real eye does not have.
"""
