#!/usr/bin/env python3
"""Native (no-emulation) port of The Sentinel's action-time line-of-sight /
sights-targeting algorithm.

This reproduces, in pure-Python integer math, the C64 ROM's player aim path
exactly (bit-for-bit), so the keyboard-win planner can call it in microseconds
instead of ~20s/emulated-aim.

It is a faithful transcription -- NOT an emulator -- of these game routines:
...
The player-aim path uses sin_cos_lookup $0E75 (polynomial). It does NOT use the
$AC80 table (that is only reached via calculate_sine_and_cosine $0F70 from the
enemy/relative-angle path $848F), so no $AC80 table is needed here.

Public entry point:
  aim_target_native(state, h_angle, v_angle, cur_x, cur_y, player_slot, eye_z=None)
      -> (tx, ty, los)
`state` is a NativeState built from live memory via NativeState.from_mem(mem).
"""
...
def _tile_byte(state, x, y):
    """calculate_tile_address $2BA8 read: tiles_table[($0024,$0026)]."""
    x &= 0xFF
    y &= 0xFF
    lo = ((x << 3) & 0xE0) | (y & 0x1F)
    page = (x & 3) + 4
...
# ============================================================================
# native state + public entry point
# ============================================================================
class NativeState:
    """Game state read directly from memory for the native LOS. Holds the
    tiles_table region ($0400-$07FF needed indices) and the object arrays."""
    ...
    def from_mem(cls, mem):
        s = cls()
        # keep a flat 64K view for tile reads (cheap: reference the memory list)
        s.mem = mem
        s.obj_x = [mem[0x0900 + i] for i in range(64)]
        s.obj_y = [mem[0x0980 + i] for i in range(64)]
        s.obj_z_height = [mem[0x0940 + i] for i in range(64)]
        s.obj_z_frac = [mem[0x0A00 + i] for i in range(64)]
        s.obj_h_angle = [mem[0x09C0 + i] for i in range(64)]
        s.obj_v_angle = [mem[0x0140 + i] for i in range(64)]
        s.obj_flags = [mem[0x0100 + i] for i in range(64)]
        s.obj_type = [mem[0x0A40 + i] for i in range(64)]
        return s


def aim_target_native(
    state,
    h_angle,
    v_angle,
    cur_x,
