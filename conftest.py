...
def board_image():
    """``(landscape, at_entry) -> bytes``: the 64 KB image of the landscape a player
    TYPES, generated once per key and memoised immutably. ``at_entry`` applies the
    ROM's entry scalars (cursor, cooldown gate, enemies-frozen flag)."""
    from sentinel import landscape as ls
    ...
    def image(number=0, at_entry=False):
        ...
        if key not in cache:
            st = (
                Game.typed(number).state
                if at_entry
                else ls.generate(ls.seed_for(number))
            )
...
def new_state(board_image):
    """``(landscape) -> State``: a fresh mutable copy of the cached board, so no test
...
def new_game(board_image):
    """``(landscape) -> Game`` at the ROM's at-entry state, over a fresh copy of the
    cached board (the cheap equivalent of ``Game.typed``)."""
    from sentinel.game import Game
    from sentinel.state import State
